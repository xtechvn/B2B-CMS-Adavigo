...
$('#btn__submit_hotel_payment').click(function () {

    var order_id = sessionStorage.getItem(hotel_constants.CONSTATNTS.STORAGE.OrderID)
    var order_id_value = $('#input__hotel_payment_orderid').val()
    ...
    let obj = {
        ...
        booking_id: $('#input__hotel_payment_bookingid').val(),
        amount: parseFloat($('#input__hotel_payment_amount').val()),
    ...
    _ajax_caller.post('/hotel/savepayment', { model: obj }, function (result) {
        if (result.isSuccess) {
            obj.order_no = result.data;
            sessionStorage.setItem(hotel_constants.CONSTATNTS.STORAGE.OrderID, result.order_id)
            if (obj.payment_type == 5) { window.location.href = '/hotel/OrderSuccess?order_no=' + obj.order_no + '&payment_type=5' }
            else {
