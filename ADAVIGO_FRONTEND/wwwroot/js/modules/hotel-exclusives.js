...
$(document).ready(function () {
    hotel_exclusives.Initialization()

})
function CheckPrice() {
    setTimeout(function () {
        var finished = hotel_exclusives.Data.length > 0 && hotel_exclusives.Data.some((x) => x.finished == undefined) ? false : true;
        if (hotel_exclusives.Data.length > 0 && finished) {
            $('#list-article-exclusive').addClass('box-placeholder')
            $('#list-article-exclusive').addClass('placeholder')
            hotel_exclusives.RenderSorted()
        }
        else {
            CheckPrice();
        }
...
var hotel_exclusives = {
    ...
    Initialization: function () {
        ...
        $('#list-article-exclusive').addClass('box-placeholder')
        $('#list-article-exclusive').addClass('placeholder')
        $('#list-article-exclusive-commit-label').hide()
        hotel_exclusives.dateSinglePicker($('.date-range-fromdate'))
        hotel_exclusives.dateSinglePicker($('.date-range-todate'))
        hotel_exclusives.RenderLocation()
        ...
        if (str) {
            ...
            hotel_exclusives.RenderListing(input)
    ...
    DynamicBind: function () {
        ...
        $("body").on('click', ".hotel-exclusive-name", function () {
            var element = $(this)
            hotel_exclusives.searchHotel(element.closest('.hotel-exclusive-item'))

        });
        $("body").on('apply.daterangepicker', ".date-range-fromdate", function (ev, picker) {
            hotel_exclusives.OnApplyStartDateOfBookingRange($('.date-range-fromdate'), $('.date-range-todate'));
    ...
    RenderListing: function (input) {
        ...
        input_2.fromdate = hotel_exclusives.ConvertToDateDotnet(input_2.fromdate)
        input_2.todate = hotel_exclusives.ConvertToDateDotnet(input_2.todate)
            ...
            if (result.isSuccess) {
                ...
                $(result.data).each(function (index, item) {
                    html += hotel_constants.HTML.ExclusiveArticle
                        ...
                        .replaceAll('{amount}', item.min_price == null || item.min_price == undefined || item.min_price <= 0 ? 'Giá liên hệ' : hotel_exclusives.Comma(Math.round(item.min_price / diffDays, 0)) + ' đ')
                ...
                hotel_exclusives.CalucateDetail(input_2, diffDays)
        ...
        $('.hotel-exclusive-item').each(function (index, item) {
            ...
            _ajax_caller.post('/hotel/GetExclusiveHotelDetail', { request_model: input }, function (result_detail) {
                if (result_detail.isSuccess) {
                    ...
                    var show_price = hotel_exclusives.Comma(Math.round(min_price / diffDays, 0))
                    ...
                    hotel_exclusives.Data.find(x => x.hotel_id == input.hotelid).min_price = min_price
                }
                else {
                ...
                hotel_exclusives.Data.find(x => x.hotel_id == input.hotelid).finished = "finished"
            })
    ...
    RenderSorted: function () {
        var diffDays = hotel_exclusives.Nights
        var has_price = hotel_exclusives.Data.filter(x => x.min_price > 0 && x.is_commit != true)
        has_price = has_price.sort((a, b) => (a.min_price - b.min_price));
        var no_price = hotel_exclusives.Data.filter(x => x.min_price <= 0 && x.is_commit != true)
        var has_price_commit = hotel_exclusives.Data.filter(x => x.min_price > 0 && x.is_commit == true)
        var no_price_commit = hotel_exclusives.Data.filter(x => x.min_price <= 0 && x.is_commit == true)
        var html=''
        var html_commit=''
        $(has_price_commit).each(function (index, item) {
            ...
            html_commit += hotel_constants.HTML.ExclusiveArticle
                .replaceAll('{city}', item.street)
                .replaceAll('{name}', item.name == null || item.name == undefined ? '' : item.name)
                .replaceAll('{name}', item.name == null || item.name == undefined ? '' : item.name)
                .replaceAll('{thumb}', item.img_thumb == null || item.img_thumb == undefined || item.img_thumb.length <= 0 ? '/images/graphics/thumb1.jpg' : item.img_thumb[0])
    ...
    dateSinglePicker: function (element) {
        $(element).daterangepicker({
            ...
            showDropdowns: true,
            autoApply: true,
            locale: hotel_exclusives.local_date_picker
        });
    },
    local_date_picker: {
